...
export default function Home() {
  ...
  const handleSwipe = (_: any, info: { offset: { x: number }; velocity: { x: number } }) => {
    ...
    if (offset.x < 0) {
      // finger dragged LEFT → go forward (right in layout): saved → browse → /game
      if (tab === 'saved') setTab('browse');
      else if (tab === 'browse') router.push('/game');
    } else {
      // finger dragged RIGHT → go back (left in layout): browse → saved → /intro
      if (tab === 'browse') setTab('saved');
      else if (tab === 'saved') router.push('/intro');
    }
  };
